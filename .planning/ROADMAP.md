...
- [ ] **Phase 1: Infrastructure Foundation** - Docker Compose stack with MongoDB, Redis, health endpoints, and all Mongoose models
- [ ] **Phase 2: Authentication & Token Management** - Azure AD OAuth 2.0 SSO, JWT sessions, encrypted token storage, multi-mailbox connections
...
|-------|---------|------------|
| 1 | Redis `allkeys-lru` silently evicts BullMQ job keys | Set `noeviction` policy in Redis config before first deployment |
| 2 | MSAL token cache lost on container restart | Implement MSAL `ICachePlugin` persisting to MongoDB; test with `docker restart` |
| 3 | Webhook subscriptions silently expire | `lifecycleNotificationUrl` on every subscription; renewal on startup + every 2h |
