...
must_haves:
  truths:
    ...
    - "Health endpoint at /api/health reports MongoDB, Redis, webhook subscription status (subscriptions.active count), and token health (tokens.healthy count)"
    - "Health endpoint returns 503 when a service is down"
    - "Health endpoint includes subscriptions.active (WebhookSubscription.countDocuments with status active) and tokens.healthy (User.countDocuments with valid encryptedTokens) -- both return 0 in Phase 1"
    ...
    - "graph.ts utility defines SELECT_FIELDS constant with required $select fields per Graph API resource type, enforcing INFR-04 for all future Graph calls"
  artifacts:
    - path: "backend/src/utils/encryption.ts"
      provides: "AES-256-GCM encrypt/decrypt functions"
      exports: ["encrypt", "decrypt"]
      contains: "aes-256-gcm"
    - path: "backend/src/utils/graph.ts"
      provides: "Graph API $select field definitions and helper for building query params"
      exports: ["SELECT_FIELDS", "buildSelectParam"]
      contains: "SELECT_FIELDS"
    ...
    - path: "backend/src/routes/health.ts"
      provides: "Health check endpoint reporting all subsystem status including webhook subscriptions and token health"
      contains: "WebhookSubscription.countDocuments"
  ...
  key_links:
    ...
    - from: "backend/src/routes/health.ts"
      to: "backend/src/models/WebhookSubscription.ts"
      via: "countDocuments for active subscriptions"
      pattern: "WebhookSubscription.countDocuments"
    - from: "backend/src/routes/health.ts"
      to: "backend/src/models/User.ts"
      via: "countDocuments for users with valid tokens"
      pattern: "User.countDocuments"
---
...
<task type="auto">
  ...
  <action>
    ...
    **backend/src/utils/graph.ts**:
    - Define and export `SELECT_FIELDS` constant object mapping Graph API resource types to their required $select field arrays:
      - message: ['id', 'subject', 'from', 'toRecipients', 'receivedDateTime', 'isRead', 'importance', 'hasAttachments', 'conversationId', 'categories', 'flag', 'internetMessageId', 'parentFolderId', 'internetMessageHeaders']
      - mailFolder: ['id', 'displayName', 'parentFolderId', 'totalItemCount', 'unreadItemCount']
      - messageRule: ['id', 'displayName', 'sequence', 'isEnabled', 'conditions', 'actions']
      - subscription: ['id', 'resource', 'changeType', 'expirationDateTime', 'notificationUrl']
    - Export `buildSelectParam(resourceType: keyof typeof SELECT_FIELDS): string` that returns a comma-joined $select string for the given resource type
    - Export `type GraphResourceType = keyof typeof SELECT_FIELDS` for type safety
    - This establishes the convention for INFR-04: all Graph API calls in Phase 3+ MUST use SELECT_FIELDS via this utility to avoid fetching unnecessary data
    - Add a JSDoc comment on SELECT_FIELDS explaining that ALL Graph API calls must use $select to minimize payload size and comply with INFR-04
    ...
    **backend/src/routes/health.ts**:
    ...
    - Import WebhookSubscription and User models from models/index.ts
    - GET /api/health:
      ...
      - Check webhook subscriptions: WebhookSubscription.countDocuments({ status: 'active' }) -- returns 0 in Phase 1, real data in Phase 3+
      - Check token health: User.countDocuments({ 'encryptedTokens.accessToken': { $exists: true }, 'encryptedTokens.expiresAt': { $gt: new Date() } }) -- returns 0 in Phase 1, real data in Phase 2+
      - Compute overall healthy: all services connected (subscriptions and tokens are informational, not gates for healthy/degraded)
      - Return status 200 if healthy, 503 if degraded
      - Response body: { status: 'healthy'|'degraded', uptime: process.uptime(), timestamp: ISO string, version: package.json version or '1.0.0', services: { mongodb: status, redis: status }, queues: { count: number of active queues }, subscriptions: { active: number }, tokens: { healthy: number } }
  ...
  <verify>
    ...
    2. `curl -s http://localhost:8010/api/health | jq .` shows MongoDB "connected", Redis "connected", subscriptions.active (number), and tokens.healthy (number)
    ...
    8. Verify graph.ts exists and exports SELECT_FIELDS: `grep -c 'SELECT_FIELDS' backend/src/utils/graph.ts` returns a positive count
    9. Backend TypeScript compiles: `cd backend && npx tsc --noEmit`
  </verify>
  <done>
    AES-256-GCM encryption encrypt/decrypt functions work with round-trip verification. Graph API $select utility (graph.ts) defines SELECT_FIELDS for all resource types, establishing the INFR-04 convention. Rate limiting enforced: 5/min on auth, 100/min on API routes, backed by Redis store. Health endpoint reports MongoDB, Redis, webhook subscription status (subscriptions.active), and token health (tokens.healthy) with 200/503 based on service health. Webhook endpoint handles Graph validation handshake and returns 202 for notifications. Security headers applied via helmet. CORS configured for frontend origin. Global error handler catches and logs all errors with structured responses.
  </done>
</task>
...
- Health endpoint reports MongoDB, Redis, uptime, queue status, webhook subscription count (subscriptions.active), and token health count (tokens.healthy)
- Webhook endpoint handles Graph API validation and returns 202
- Graph API $select utility (graph.ts) defines SELECT_FIELDS per resource type for INFR-04 compliance
