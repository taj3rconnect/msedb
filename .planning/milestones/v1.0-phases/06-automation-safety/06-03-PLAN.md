...
must_haves:
  truths:
    ...
    - "Staging API lists staged emails and supports rescue, batch rescue, execute-now, and batch execute"
  ...
  artifacts:
    ...
    - path: "backend/src/routes/staging.ts"
      provides: "Staging list, rescue, batch rescue, execute-now, batch execute"
...
<task type="auto">
  ...
  <action>
    ...
    **2. Modify `backend/src/routes/patterns.ts` to auto-create rule on approval:**
    - Import `convertPatternToRule` from `../services/ruleConverter.js`
    - In the `POST /api/patterns/:id/approve` handler, after `pattern.status = 'approved'` and `pattern.approvedAt = new Date()` and the save/auditlog, add a call to `convertPatternToRule(pattern._id, userId)` wrapped in a try-catch (rule creation failure should not fail the approve response -- log warning and continue)
    - Similarly update the `POST /api/patterns/:id/customize-and-approve` handler with the same auto-conversion after approval
    - This ensures ROADMAP success criterion 1: "User approves a pattern and a multi-action rule is created" happens automatically without requiring a separate UI action

    **3. Add whitelist endpoints to `backend/src/routes/mailbox.ts`:**
...
<task type="auto">
  ...
  <action>
    ...
    **POST /api/staging/:id/execute** -- Execute a single staged email immediately (before grace period expires)
    - Find StagedEmail by id and userId, status must be 'staged'
    - Throw NotFoundError if not found or not in 'staged' status
    - Get access token and mailbox email for the item's mailboxId
    - Execute the staged action: for 'delete' actions, move email from staging folder to Deleted Items via `POST /users/{email}/messages/{messageId}/move` with `{ destinationId: 'deleteditems' }`
    - Update `item.status = 'executed'`, `item.executedAt = new Date()`, save
    - Create AuditLog entry: action 'email_executed', targetType 'email', targetId messageId, details including ruleId, actions, originalFolder, undoable true
    - Return `{ stagedEmail }`

    **POST /api/staging/batch-execute** -- Batch execute staged emails immediately
    - Body: `{ ids: string[] }`
    - Find all StagedEmails by ids and userId, status 'staged'
    - Process each item same as single execute above (with concurrency limit of 5 using Promise.allSettled chunks)
  ...
  <done>
    Staging API with list, count, rescue, batch rescue, execute-now, and batch execute endpoints.
...
- Staging API supports list with count, rescue, batch rescue, execute-now, and batch execute
