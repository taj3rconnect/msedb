...
must_haves:
  ...
  artifacts:
    ...
    - path: "frontend/Dockerfile"
      provides: "Multi-stage Vite build to nginx with non-root"
...
<task type="auto">
  ...
  <action>
    ...
    **docker-compose.yml** (NO version field -- it is obsolete):
    ...
    - `msedb-frontend`: build from `./frontend`, port 3010:80, depends_on backend (condition: service_healthy), network msedb-network, restart unless-stopped, deploy limits cpus 0.5 memory 512M, healthcheck using wget to http://localhost:80 (interval 30s, timeout 5s, retries 3)
    ...
    **frontend/Dockerfile** (multi-stage, Vite -> nginx):
    - Stage 1 (builder): FROM node:22-alpine, WORKDIR /app, COPY package*.json, RUN npm ci, COPY . ., RUN npm run build
    - Stage 2 (runtime): FROM nginx:alpine, RUN apk add --no-cache wget, COPY --from=builder /app/dist to /usr/share/nginx/html, COPY nginx.conf to /etc/nginx/conf.d/default.conf, EXPOSE 80, CMD nginx -g daemon off
...
<task type="auto">
  ...
  <verify>
    ...
    6. `docker compose exec msedb-redis redis-cli CONFIG GET maxmemory-policy` returns "noeviction"
  </verify>
  <done>
