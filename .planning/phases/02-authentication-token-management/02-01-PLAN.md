...
<task type="auto">
  ...
  <action>
    ...
    4. Update `backend/src/models/Mailbox.ts`:
       ...
       - Add sparse unique index: `mailboxSchema.index({ homeAccountId: 1 }, { unique: true, sparse: true })`
...
<task type="auto">
  ...
  <action>
    ...
    2. Create `backend/src/auth/routes.ts`:
       ...
       **GET /auth/callback**:
       ...
       - If `action === 'login'`:
         ...
         - Now persist MSAL cache to the mailbox: Create a per-mailbox MSAL client via `createMsalClient(mailbox._id.toString())`, then call `acquireTokenByCode` again...
         - **Simpler approach:** After creating/finding the mailbox, directly save the MSAL token cache from the login client to the mailbox's msalCache field. Use `loginMsalClient.getTokenCache().serialize()` and store it on the Mailbox document.
    ...
    3. Update `backend/src/server.ts`:
       ...
       - Apply requireAuth to protected API routes: `app.use('/api', requireAuth)` (after rate limiter, before any API route handlers). Note: health endpoint is already mounted before rate limiters, so it stays unprotected.
  </action>
  <verify>
    Run from the backend directory:
    ```bash
  ...
  <done>
    ...
    - All /api/* routes protected by requireAuth (except /api/health which is mounted before)
  </done>
</task>
...
5. `grep -r "requireAuth" backend/src/server.ts` -- API routes are protected
...
- requireAuth middleware validates JWT on all /api/* routes
