...
must_haves:
  truths:
    - "After docker restart, Graph API access continues without re-authentication for all connected mailboxes"
    - "When refresh token expires, user sees mailbox as disconnected with a high-priority notification prompting reconnection"
...
<task type="auto">
  ...
  <action>
    ...
    2. Create `backend/src/jobs/processors/tokenRefresh.ts`:
       ...
       - Export async function `processTokenRefresh(job: Job): Promise<void>`:
         ...
         - Query ALL connected mailboxes (no expiry filter). MSAL's `acquireTokenSilent()` will return cached tokens immediately when they are still valid, so there is no performance penalty for checking all mailboxes. This avoids the risk of a delayed job cycle missing a narrow expiry window:
           ```typescript
    ...
    3. Update `backend/src/jobs/queues.ts`:
       ...
       - **Remove the existing `const workers: Worker[] = QUEUE_NAMES.map(...)` block entirely** (lines ~60-79 in the current file, including all worker event handlers inside it). This block will be replaced with processorMap-based creation below.
       - Create a processorMap to assign real or placeholder processors per queue:
         ```typescript
  ...
  <done>
    - Token refresh processor queries all connected mailboxes and calls acquireTokenSilent via MSAL (no expiry filter -- MSAL handles cache hits for still-valid tokens)
...
<task type="auto">
  ...
  <verify>
    ...
    ```
    Verify the reconnect branch (existing-but-disconnected mailbox) is present in callback:
    ```bash
    grep -A2 "isConnected.*false\|disconnected\|reconnect" backend/src/auth/routes.ts
    ```
  </verify>
  <done>
