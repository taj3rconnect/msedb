...
must_haves:
  truths:
    - "Rule engine checks kill switch first, whitelist second, then evaluates rules by priority"
    - "Destructive actions (delete) are routed to staging folder, not executed immediately"
    - "Whitelisted senders/domains are never auto-acted on"
    - "StagedEmail documents persist beyond expiresAt for processor to execute them"
  artifacts:
