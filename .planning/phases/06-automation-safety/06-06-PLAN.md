...
must_haves:
  truths:
    ...
    - "User can execute staged emails immediately via Execute Now button"
  ...
  artifacts:
    ...
    - path: "frontend/src/api/staging.ts"
      ...
      exports: ["fetchStagedEmails", "fetchStagingCount", "rescueStagedEmail", "batchRescueStagedEmails", "executeStagedEmail", "batchExecuteStagedEmails"]
...
<task type="auto">
  ...
  <action>
    **1. Create `frontend/src/api/staging.ts`:**
    ...
    - `executeStagedEmail(id: string): Promise<{ stagedEmail: StagedEmail }>` -- POST /api/staging/:id/execute
    - `batchExecuteStagedEmails(ids: string[]): Promise<{ executed: number }>` -- POST /api/staging/batch-execute
    ...
    **2. Create `frontend/src/hooks/useStaging.ts`:**
    ...
    - `useExecuteStagedEmail()` -- useMutation calling executeStagedEmail, invalidates ['staging', 'staging-count']
    - `useBatchExecute()` -- useMutation calling batchExecuteStagedEmails, invalidates ['staging', 'staging-count']
    ...
    **StagedEmailRow component (inline):**
    - Props: `{ email: StagedEmail; selected: boolean; onSelect; onRescue; onExecute }`
    - Show: checkbox | message ID (truncated) | original folder | actions badges | countdown timer | rescue button | execute now button
    - Countdown timer with color-coded badge using useCountdown
    - Rescue button (lifesaver icon) triggers onRescue
    - "Execute Now" button (Play/Zap icon from lucide-react) triggers onExecute -- forces immediate execution of the staged action before the 24-hour grace period expires. Use a confirmation dialog (shadcn AlertDialog) before executing since this is a destructive action.
    ...
    **StagingPage component:**
    ...
    - Batch execute: "Execute Selected" button next to "Rescue Selected" (with confirmation dialog)
